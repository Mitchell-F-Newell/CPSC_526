...
int main(int argc, char *argv[]) {
    ...
    if(argc != 6) {
        ...
        printf("<source hostname/IP> <source port> <TCP seq Number> <target hostname/IP> <target port>\n");
        exit(-1);
    }
    ...
    cs.dest_address = inet_addr( argv[4] );
    cs.placeholder = 0;
    ...
    printf("Using: Source IP: %s port: %u, Spoof Seq: %u, Target IP: %s port: %u.\n", argv[1], atoi(argv[2]), atoi(argv[3]), argv[4], atoi(argv[5]));
